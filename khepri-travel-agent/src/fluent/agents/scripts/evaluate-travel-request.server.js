// IIFE tool script for Evaluate Travel Request
...
(function(inputs) {
    try {
        ...
        if (!reqGr.next()) {
            return JSON.stringify({ success: false, error: 'Travel request ' + requestNumber + ' not found' });
        }
        ...
        var requestData = {
            ...
            accommodation_per_night: '$' + (reqGr.getValue('estimated_accommodation_per_night') || '0'),
            total_cost: '$' + (reqGr.getValue('total_estimated_cost') || '0')
        };
        ...
        while (ruleGr.next()) {
            ...
            if (matched) {
                ...
                if (ruleId === 'RULE_ACCOM_DOM' && travelType !== 'domestic') { continue; }
                if (ruleId === 'RULE_ACCOM_INTL' && travelType !== 'international') { continue; }
        ...
        var policyAssessment = assessmentParts.length > 0
            ...
            : 'All items within policy guidelines.';

        var approvalStatus = 'pending_review';
        if (highestLevel === 'vp') { approvalStatus = 'pending_vp'; }
        else if (highestLevel === 'manager') { approvalStatus = 'pending_manager'; }
        ...
        var nextSteps = [];
        if (highestLevel === 'vp') {
            nextSteps.push('VP approval required. Forward to the appropriate VP with the policy assessment.');
        } else if (highestLevel === 'manager') {
            nextSteps.push('Manager approval required. Forward to the direct manager.');
        } else {
            nextSteps.push('No additional flags. Standard manager routing applies.');
        }
        if (flags.length > 0) {
            nextSteps.push(flags.length + ' item(s) flagged. Approver should review flags before approving.');
        }
        ...
        return JSON.stringify({
            ...
            evaluation_result: {
                ...
                requirements_count: approvalRequirements.length
            },
        ...
        return JSON.stringify({
            success: false,
            error: 'Evaluate tool failed: ' + e.message,
            request_number: inputs.request_number || '(none)'
        });
    }
})(inputs);
