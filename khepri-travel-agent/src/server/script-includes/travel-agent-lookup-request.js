// @ts-nocheck
var TravelAgentLookupRequest = Class.create();
TravelAgentLookupRequest.prototype = {
    initialize: function() {},

    lookupRequest: function(requestNumber, requesterEmail) {
        try {
            ...
            if (requestNumber) {
                // Lookup by request number - return single detailed record
                ...
                if (!gr.next()) {
                    return JSON.stringify({
                        success: false,
                        error: 'Travel request ' + requestNumber + ' not found in table x_snc_travel_a7t2p_travel_request'
                    });
                }

                var sysId = gr.getUniqueValue();
                var recordLink = baseUrl + 'x_snc_travel_a7t2p_travel_request.do?sys_id=' + sysId;
                var record = this._recordToObject(gr);
                record.record_link = recordLink;
                ...
                // Lookup by email - may return multiple
                ...
                var listLink = baseUrl + 'x_snc_travel_a7t2p_travel_request_list.do?sysparm_query=requester_email=' + requesterEmail;
            ...
            gs.error('[TravelAgent] LookupRequest error: ' + e.message);
            return JSON.stringify({
                success: false,
                error: 'Failed to look up travel request: ' + e.message,
